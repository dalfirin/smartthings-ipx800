...
metadata {
    ...
    multiAttributeTile(name: "videoPlayer", type: "videoPlayer", width: 6, height: 4) {
			tileAttribute("device.switch5", key: "module_STATUS") {
				attributeState("on", label: "Active", icon: "st.module.dlink-indoor", action: "vidOff", backgroundColor: "#79b821", defaultState: true)
				attributeState("off", label: "Inactive", icon: "st.module.dlink-indoor", action: "vidOn", backgroundColor: "#ffffff")
				attributeState("restarting", label: "Connecting", icon: "st.module.dlink-indoor", backgroundColor: "#53a7c0")
				attributeState("unavailable", label: "Unavailable", icon: "st.module.dlink-indoor", action: "refresh.refresh", backgroundColor: "#F22000")
			}

			tileAttribute("device.errorMessage", key: "module_ERROR_MESSAGE") {
				attributeState("errorMessage", label: "", value: "", defaultState: true)
			}

			tileAttribute("device.module", key: "PRIMARY_CONTROL") {
				attributeState("on", label: "Active", icon: "st.module.dlink-indoor", backgroundColor: "#79b821", defaultState: true)
				attributeState("off", label: "Inactive", icon: "st.module.dlink-indoor", backgroundColor: "#ffffff")
				attributeState("restarting", label: "Connecting", icon: "st.module.dlink-indoor", backgroundColor: "#53a7c0")
				attributeState("unavailable", label: "Unavailable", icon: "st.module.dlink-indoor", backgroundColor: "#F22000")
			}
    	...
    	carouselTile("moduleDetails", "device.image", width: 3, height: 2) { }
        standardTile("take", "device.image", width: 2, height: 2, canChangeIcon: false, inactiveLabel: true, canChangeBackground: false) {
            state "take", label: "Take", action: "Image Capture.take", icon: "st.module.module", backgroundColor: "#FFFFFF", nextState:"taking"
            state "taking", label:'Taking', action: "", icon: "st.module.take-photo", backgroundColor: "#53a7c0"
            state "image", label: "Take", action: "Image Capture.take", icon: "st.module.module", backgroundColor: "#FFFFFF", nextState:"taking"
        }
       ...
       controlTile("levelSliderControl", "device.level", "slider", height: 1, width: 5, inactiveLabel: false, range:"(0..100)") {
        ...
        main "motion"
        details(["videoPlayer","moduleDetails", "take","refresh","videoStart","levelSliderControl","Sensitivity","motion","PIR","nightVision", "Video"])
    }
}
...
// handle commands
def take() {
	def userpassascii = "${state.moduleUser}:${state.modulePassword}"
	def userpass = "Basic " + userpassascii.encodeAsBase64().toString()
    def host = state.moduleIP 
    def hosthex = convertIPtoHex(host)
    def porthex = convertPortToHex(state.modulePort)
    ...
    def headers = [:] 
    headers.put("HOST", "$host:$state.modulePort")
...
def start() {
	log.trace "start()"
	def dataLiveVideo = [
		OutHomeURL  : "rtsp://${state.moduleUser}:${state.modulePassword}@${state.videoIP}:${state.videoPort}/live1.sdp",
		InHomeURL   : "rtsp://${state.moduleUser}:${state.modulePassword}@${state.videoIP}:${state.videoPort}/live1.sdp",
		ThumbnailURL: "http://cdn.device-icons.smartthings.com/module/dlink-indoor@2x.png",
...
def configure(){
    ...
    state.moduleIP = parent.state.moduleIP
    state.modulePort = parent.state.modulePort
    state.videoIP = parent.state.VideoIP
    state.videoPort = parent.state.VideoPort
    state.moduleUser = parent.state.moduleUser
    state.modulePassword = parent.state.modulePassword 
}
